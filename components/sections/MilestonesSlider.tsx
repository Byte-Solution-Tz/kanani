          ...
          {milestones.map((milestone) => (
            ...
            >
              <div className="pb-7 text-center">
                <p className="text-4xl font-black text-[#f90000]">{milestone.year}</p>
                <p className="mt-2 text-sm font-semibold text-[var(--kanan-muted-strong)]">{milestone.title}</p>
                <span className="mt-4 inline-flex h-7 w-7 items-center justify-center rounded-full border-[6px] border-[#f90000] bg-white shadow-[0_0_0_4px_rgba(116,191,159,0.24)]" />
              </div>

              <div className="theme-panel relative overflow-hidden rounded-2xl border border-[#fb0000] bg-white p-3">
                <span className="absolute left-1/2 top-0 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rotate-45 border-l border-t border-[#f60000] bg-white" />
        ...
        {Array.from({ length: maxIndex + 1 }).map((_, index) => (
          <button
            ...
            className={`h-2.5 rounded-full transition ${
              index === clampedActiveIndex ? "w-9 bg-[#ff0000]" : "w-6 bg-[#c5d1ce] hover:bg-[#9fc7b9]"
            }`}
          />
        ))}
      </div>
