      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-12 lg:gap-10">
          <div className="lg:col-span-5">
            ...
            <div className="mt-3 h-1 w-40 bg-[var(--kanan-brand-red)]" />
            <h2 className="mt-4 text-4xl font-black leading-tight text-[var(--kanan-text)] sm:text-5xl">
              Industries We Serve
            </h2>
          </div>
          <div className="lg:col-span-7">
            <p className="text-base leading-8 text-[var(--kanan-muted-strong)]">
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {industries.map((industry) => (
            <article
              key={industry.title}
              className="theme-panel rounded-xl border border-[var(--kanan-line)] px-5 py-6"
            >
              <div className="mb-4 inline-flex h-11 w-11 items-center justify-center rounded-lg bg-[var(--kanan-bg-soft)] text-[var(--kanan-brand-blue)]">
                <industry.icon className="h-5 w-5" />
              </div>
              <h3 className="text-xl font-black text-[var(--kanan-text)]">{industry.title}</h3>
              <p className="mt-3 text-sm leading-7 text-[var(--kanan-muted-strong)]">{industry.description}</p>
              <span className="mt-5 inline-flex items-center rounded-full bg-[var(--kanan-bg-soft)] px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-[var(--kanan-brand-blue)]">
                {industry.stat}
              </span>
            </article>
          ))}
        ...
        <div className="theme-panel mt-12 flex flex-col items-start justify-between gap-4 rounded-xl border border-[var(--kanan-line)] px-6 py-6 md:flex-row md:items-center">
          <div>
            <p className="text-2xl font-black text-[var(--kanan-text)]">Need a custom supply plan?</p>
            <p className="mt-1 text-sm text-[var(--kanan-muted)]">
